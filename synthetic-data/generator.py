...
def sample_age_from_band(band: str, random_generator: np.random.Generator) -> int:
    ranges = {
        '0-17': (0, 17),
        '18-24': (18, 24),
        '25-34': (25, 34),
...
def main():
    ...
    # Sample covariates
    gender = random.choice(genders, size=TOTAL, p=gender_p)
    location = random.choice(locations, size=TOTAL, p=loc_p)
    drug = (random.random(TOTAL) < p_drug).astype(int)
    mental = (random.random(TOTAL) < p_mental).astype(int)
    indigenous = (random.random(TOTAL) < p_indigenous).astype(int)
    dv = (random.random(TOTAL) < p_dv).astype(int)

    # Choose from age range
    age_band = random.choice(ages, size=TOTAL, p=age_p)
    age_numeric = np.array([sample_age_from_band(b, random) for b in age_band], dtype=int)
    ...
    homeless = (random.random(TOTAL) < p_h).astype(int)
